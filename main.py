...
def workshop_adoption(
    ...
    """Return all active workshops with driver adoption counts.

    Deliberately uses one LEFT JOIN + GROUP BY query instead of per-workshop
    correlated queries. This is more portable across PostgreSQL versions and
    avoids the parameter/correlation issue that was causing HTTP 500s.
    """
    fd, td, _, ss, pt = scope_values(from_date, to_date, None, sso, partner_type)

    clauses = ["w.is_active = TRUE"]
    params = {}
    if ss:
        clauses.append("w.sso = :sso")
        params["sso"] = ss
    if pt:
        clauses.append("w.partner_type = :partner_type")
        params["partner_type"] = pt
    if fd:
        clauses.append("d.registration_date >= :from_date")
        params["from_date"] = fd
    if td:
        clauses.append("d.registration_date <= :to_date")
        params["to_date"] = td
    ...
    sql = f"""
                  ...
                  OR EXISTS (
                      ...
                      WHERE mp.driver_id = d.id
                        AND (
                            ...
                            OR COALESCE(mp.pretest_attempts, 0) > 0
                        )
                  )
                THEN d.id
            END) AS started,
            COUNT(DISTINCT CASE
                WHEN COALESCE(d.completion_percentage, 0) >= 100
                  OR EXISTS (
                      ...
                      GROUP BY mp2.driver_id
                      HAVING COUNT(DISTINCT mp2.module_code) >= 4
                  )
                THEN d.id
            END) AS completed,
            COUNT(DISTINCT CASE
        ...
        LEFT JOIN public.drivers d
          ON d.workshop_id = w.id
         AND {(' AND '.join([x for x in clauses if x.startswith('d.registration_date')]) if any(x.startswith('d.registration_date') for x in clauses) else 'TRUE')}
        WHERE {' AND '.join([x for x in clauses if not x.startswith('d.registration_date')])}
        GROUP BY
            w.id, w.workshop_code, w.workshop_name,
            w.city, w.sso, w.region, w.partner_type
    ...
    try:
        ...
        db.rollback()
        raise HTTPException(500, f"Workshop Adoption query failed: {e}")

    out = []
    for w in rows:
        registered = int(w["registered"] or 0)
        out.append({
            ...
            "started": int(w["started"] or 0),
            "completed": int(w["completed"] or 0),
            "certified": int(w["certified"] or 0),
