...
from sqlalchemy import create_engine, String, Boolean, DateTime, Integer, Numeric, ForeignKey, select, func, or_
from sqlalchemy.orm import DeclarativeBase, Mapped, mapped_column, Session, sessionmaker

# LIVE, READ-ONLY DTM REPORTING SERVICE
# The driver application writes to Supabase PostgreSQL.
# This service reads that SAME PostgreSQL database through DATABASE_URL.
# It never creates tables, seeds data, or writes driver data.

DATABASE_URL = os.getenv("DATABASE_URL") or os.getenv("SUPABASE_DB_URL")
if not DATABASE_URL:
    raise RuntimeError("DATABASE_URL is not configured. Set it to the SAME Supabase PostgreSQL connection string used by DTM.")

# Accept Supabase/Render-style PostgreSQL URLs.
...
# Supabase pooled/direct URLs generally require SSL. If sslmode is already present, keep it.
...
class Workshop(Base):
    ...
    workshop_division: Mapped[Optional[str]] = mapped_column(String(255))
...
app = FastAPI(title="Tata Motors DTM Management Reporting API", version="3.0.0")
...
def scope_params(from_date, to_date, workshop_code, sso, partner_type):
    fd, td = parse_date(from_date), parse_date(to_date, True)
...
def driver_scope(fd=None, td=None, wc=None, ss=None, pt=None):
...
def pct(n, d):
    return round(float(n) / float(d) * 100, 1) if d else 0
...
def health():
    try:
        with SessionLocal() as s:
            s.execute(select(func.count()).select_from(Driver)).scalar()
        return {"ok": True, "service": "DTM Management Reporting", "version": "3.0.0", "database": "connected", "database_source": "supabase_postgresql"}
    except Exception as e:
        return {"ok": False, "service": "DTM Management Reporting", "version": "3.0.0", "database": "error", "detail": str(e)}
...
def filters(s: Session = Depends(db)):
    ...
    workshops = s.scalars(select(Workshop).where(Workshop.is_active == True).order_by(Workshop.workshop_name, Workshop.city)).all()
    return {
        "ssos": ssos,
        "partner_types": partners,
        "workshops": [{"workshop_code": w.workshop_code, "workshop_name": w.workshop_name, "city": w.city} for w in workshops],
...
def dashboard_api(from_date: Optional[str]=None, to_date: Optional[str]=None, workshop_code: Optional[str]=None, sso: Optional[str]=None, partner_type: Optional[str]=None, s: Session=Depends(db)):
    ...
    completed_ids = select(ModuleProgress.driver_id).join(ids, ModuleProgress.driver_id == ids.c.id).where(ModuleProgress.status.in_(["Completed", "completed"])).group_by(ModuleProgress.driver_id).having(func.count(func.distinct(ModuleProgress.module_code)) >= 4).subquery()
    ...
    return {"filters":{"from_date":from_date,"to_date":to_date,"workshop_code":wc,"sso":ss,"partner_type":pt},"kpis":{
        "registered_drivers": int(total), "training_started": int(started), "training_completed": int(completed), "modules_completed": int(completed), "in_progress": max(int(total-completed),0), "certified_drivers": int(certified),
...
def module_progress(from_date: Optional[str]=None, to_date: Optional[str]=None, workshop_code: Optional[str]=None, sso: Optional[str]=None, partner_type: Optional[str]=None, s: Session=Depends(db)):
    fd, td, wc, ss, pt = scope_params(from_date,to_date,workshop_code,sso,partner_type)
    ids = driver_scope(fd,td,wc,ss,pt).subquery(); total = s.scalar(select(func.count()).select_from(ids)) or 0
    out=[]
    for code,name in MODULES.items():
        rows=s.scalars(select(ModuleProgress).join(ids,ModuleProgress.driver_id==ids.c.id).where(ModuleProgress.module_code==code)).all()
        started=sum(1 for r in rows if r.started_at); completed=sum(1 for r in rows if str(r.status).lower()=="completed"); attempts=sum(int(r.pretest_attempts or 0) for r in rows); scores=[float(r.pretest_percentage or 0) for r in rows if int(r.pretest_attempts or 0)>0]
...
def workshop_adoption(from_date: Optional[str]=None, to_date: Optional[str]=None, sso: Optional[str]=None, partner_type: Optional[str]=None, s: Session=Depends(db)):
    ...
    ws=s.scalars(wq.order_by(Workshop.workshop_name,Workshop.city)).all(); out=[]
    for w in ws:
        q=select(Driver).where(Driver.workshop_id==w.id)
        if fd:q=q.where(Driver.registration_date>=fd)
        if td:q=q.where(Driver.registration_date<=td)
        ds=s.scalars(q).all(); registered=len(ds); started=sum(1 for d in ds if str(d.overall_status).lower() not in ("registered","")); certified=sum(1 for d in ds if d.certificate_id); completed=0
        if ds:
            dids=[d.id for d in ds]; rows=s.execute(select(ModuleProgress.driver_id,func.count(func.distinct(ModuleProgress.module_code))).where(ModuleProgress.driver_id.in_(dids),ModuleProgress.status.in_(["Completed","completed"])).group_by(ModuleProgress.driver_id)).all(); completed=sum(1 for _,n in rows if n>=4)
...
def drivers_report(from_date:Optional[str]=None,to_date:Optional[str]=None,workshop_code:Optional[str]=None,sso:Optional[str]=None,partner_type:Optional[str]=None,search:Optional[str]=None,limit:int=Query(500,ge=1,le=5000),s:Session=Depends(db)):
    ...
    for d,w in rows:
        ...
        out.append({"driver_id":d.driver_id,"full_name":d.full_name,"mobile_number":d.mobile_number,"licence_number":d.licence_number,"vehicle_number":d.vehicle_number,"vehicle_model":d.vehicle_model,"workshop_name":d.workshop_name or (w.workshop_name if w else None),"sso":w.sso if w else None,"partner_type":w.partner_type if w else None,"status":d.overall_status,"completion_percentage":float(d.completion_percentage or 0),"registration_date":json_dt(d.registration_date),"last_activity":json_dt(d.last_activity),"certificate_id":d.certificate_id,"final_score":finals.score if finals else None,"final_percentage":float(finals.percentage) if finals else None,"final_status":finals.status if finals else None,"modules":md})
    return out
